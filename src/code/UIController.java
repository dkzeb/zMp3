                ...
                trackList.setOnMouseClicked(new EventHandler<MouseEvent>() {
                    ...
                    public void handle(MouseEvent event) {
                        ...
                        if (event.getClickCount() == 2 && !event.isConsumed()) {
                            ...
                            // artist, title and album display text
                            if(artistTitleText != null || albumText != null) {
                                artistTitleText.setText(Main.getMediaInfo("artTrk"));
                                albumText.setText(Main.getMediaInfo("album"));
                            }

                            System.out.println("AlbumArt: "+albumArt+" from main: "+Main.getAlbumArt());
                            if(albumArt != null){
                                    albumArt.setImage(Main.getAlbumArt()); // = new ImageView(Main.getAlbumArt());
                            }
                            Main.setSelectedAndPlay(selectedFile);
                            Main.getMP().setOnEndOfMedia(new Runnable() {
                                @Override
                                public void run() {
                                    playNxtTrack();
                                    artistTitleText.setText(Main.getMediaInfo("artTrk"));
                                    albumText.setText(Main.getMediaInfo("album"));
                                    trackList.getSelectionModel().select(trackIndex);
                                    trackList.getFocusModel().focus(trackIndex);
                                    trackList.scrollTo(trackIndex);
                                }
                            });
                        } else {
